...
for i in range(5): #You can change the number '5' according to your number of images

    frame = cv2.imread(''+str(i+1)+'.jpeg') #Read images
    frame = imutils.resize(frame, width=480) #Set the width as 480
    ...
    #Next 2 lines help us to detect where are the all faces in the image
    ...
    for i in range(0, detections.shape[2]): #Loop over detections
        ...
        if probability > 0.5: #No need to check weak probabilities
            ...
            #face = np.expand_dims(face, axis=0) #Check line 50
    ...
    if len(facelist) > 0:
        facelist = np.array(facelist) #If you want to read just one face, delete this line and use line 43rd
